"""pipeline/contracts.py — re-exports all contracts + _CONF_RANK for test imports."""
from typing import Any

try:
    from ..models.contracts import *  # noqa: F401, F403
    from ..models.contracts import (
        BiometricInput, BiometricResult, HoneypotResult, GovernorResult,
        WatchdogResult, PipelineOutput, Confidence, HoneypotVerdict,
        SecurityPreset, WatchdogAction, TenantPolicy, GovernorAction,
        BOT_THETA_HARD, BOT_THETA_SOFT,
        EREC_WARN, EREC_CRITICAL,
        TRUST_WARN, TRUST_CRITICAL,
        SERVER_LOAD_HIGH,
    )
except ImportError:
    from models.contracts import *  # type: ignore # noqa: F401, F403
    from models.contracts import (  # type: ignore
        BiometricInput, BiometricResult, HoneypotResult, GovernorResult,
        WatchdogResult, PipelineOutput, Confidence, HoneypotVerdict,
        SecurityPreset, WatchdogAction, TenantPolicy, GovernorAction,
        BOT_THETA_HARD, BOT_THETA_SOFT,
        EREC_WARN, EREC_CRITICAL,
        TRUST_WARN, TRUST_CRITICAL,
        SERVER_LOAD_HIGH,
    )

# Backwards-compatible alias used by older orchestrator imports. The concrete
# challenge class lives in stage2_honeypot, so avoid importing it here during
# package initialization.
ChallengePayload = Any

# test_pipeline.py imports _CONF_RANK from pipeline.contracts
try:
    from ..models.orchestrator import _CONF_RANK  # noqa: F401
except ImportError:
    from models.orchestrator import _CONF_RANK  # type: ignore # noqa: F401
